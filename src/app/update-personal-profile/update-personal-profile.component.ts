...
export class UpdatePersonalProfileComponent implements OnInit {
 ...
 Tab2: any='Business Details';
  ...
  updatePersonalDetails(Firstname,Othernames,Surname,DOB,Gender,LevelOfEducation,NatIdNo,PostalAddress,MobileNo,
    AlternativePhoneNumber, EmailAddress,Empstatus,personalKRA){
      this.spinnerContent='Udating Profile....';
      this.spinner.show();

      this.userService.updateMemberProfile(this.MemberId,MobileNo, AlternativePhoneNumber,EmailAddress,LevelOfEducation,PostalAddress,personalKRA).subscribe((Response)=>{
      ...
      this.isSuccess = this.appRespData['IsSuccess'];
        ...
        if (this.isSuccess==false && this.errDescription!=''){
          ...
          if(Response.Individualmember.personalKRA != null){this.personalKRA=Response.Individualmember.personalKRA;}
            ...
            this.NatIdNo = localStorage.getItem('natIdNo');
            console.log('UpdateNatId'+this.NatIdNo);
  ...
  updateBusinessDetails(BName,BRegDate,BKRA,BForm, BType,LegalStatus,RegistrationNo,BusinessPhysicalAddress){
    this.spinnerContent='Updating Business Profile....';
    this.spinner.show();

    this.userService.updateMemberBusinessDetails(this.MemberId,BName,LegalStatus,BForm,RegistrationNo,BRegDate,BKRA,BusinessPhysicalAddress,BType).subscribe((Response)=>{
      this.appRespData=Response;
