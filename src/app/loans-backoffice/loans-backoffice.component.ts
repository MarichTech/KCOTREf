...
@Component({
  selector: 'app-loans-backoffice',
  templateUrl: './loans-backoffice.component.html',
  styleUrls: ['./loans-backoffice.component.css']
})
export class LoansBackofficeComponent implements OnInit {
  Terms:string;
  resData:any;
  ...
  enteredPeriod:any;
  loanProducts:any;
  fetchingLoanTypes:any;
  LoanTypeId:any;
  hideProductSelection:any;
  fetchingLoanType:any;
  uploadingLoan:any;
  ...
  constructor(private lenderService:UserService, private alertCtrl:AlertService,private navCtrl:NgxNavigationWithDataComponent,private spinner:NgxSpinnerService) { 
    ...
    this.spinnerContent="";
    this.fetchingLoanTypes=true;
    this.hideProductSelection=false;
    this.uploadingLoan=false;
  }
  ...
  getLoanTypes(){
    this.lenderService.getLoanTypes().subscribe((Response)=>{
      this.Connecting=false;
      console.log('fetching loantypes');
      console.log(Response);
      this.loanProducts=Response;
      this.fetchingLoanTypes=false;
  },(error)=>{
    this.Connecting=false;
 ...
 /* getNewLoanGoAhead(){
    ...
    this.ineligibilityReason=window.localStorage.getItem('ineligibilityReason');
    this.isEligible=window.localStorage.getItem('isEligible');
    console.log('isEligible='+this.isEligible);
    console.log('ineligibilityReason='+this.ineligibilityReason);
  }*/
  getStoredMemberId(){
  ...
  getLoanLimit(memberId){
      this.lenderService.getMemberLoanEligibility(memberId).subscribe((Response)=>{
        console.log(Response);
        this.loanlimit=Response[0];
        this.loanlimit=this.loanlimit["MemberEligibility"];
        console.log('eligibility '+this.loanlimit);
      },(error)=>{
        this.Connecting=false;
      this.disconnected=true;
      return;
...
if (Period==0||Period==undefined){
  ...
  ConfirmedApplication(){
    this.uploadingLoan=true;
    this.lenderService.addCustLoan(this.resMemberId,this.LoanTypeId,this.Period,this.principal,this.instalment) .subscribe((Response)=>{
      this.uploadingLoan=false;
        ...
        if (this.respIsSuccess==true){
              this.navCtrl.navigate('LoanResult',{
                status:'success',
                subTitle:"SUCCESS!",
                message:'Loan Request of Kes. '+this.principal+' was uploaded successfully'
              });
        }
  ...
  LoanSchedule(LoanTypeId,LoanAmount,Period){
    ...
    if(LoanAmount&&LoanAmount>0&&LoanAmount<=this.resMaximumAmount&&LoanAmount>=this.resMinimumAmount&&LoanAmount<=this.loanlimit&&Period&&Period>0&&Period<=this.resPeriod){
    this.generatingPaySchedule=true;
